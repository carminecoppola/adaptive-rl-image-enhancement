...
def test_aggressive_blend_vs_individual(underwater_image):
    ...
    assert result_aggressive.shape == underwater_image.shape
    assert torch.isfinite(result_aggressive).all()
    assert not torch.equal(result_aggressive, underwater_image)
